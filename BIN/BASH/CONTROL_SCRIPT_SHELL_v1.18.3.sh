#!/bin/bash
#Control Script Version 1.18.3
#New in v1.18.3:
#  - Added schema verification to database_import after successful impdp (STC15-776)
#  - Spools dba_users query to log and parses result to confirm schema exists (STC15-778)
#  - Sends failure email if schema is not found after import (STC15-777)
#  - Added edge case check for missing spool log (STC15-775)
#---------------------------------------------------------------
...
#Schema Verification
#Log into the target database and confirm the remapped schema exists in dba_users
...
#Connect to sqlplus and spool the dba_users query result to log
...
#Verify the spool log was created before grepping it
...
#Parse the spool log to confirm the schema was found
...
#Column 5 is Use% in df -h output
...
FUNCTION_OPTIONS="secure_copy backup_f_d database_backup database_import disk_utilization cleanup local_migration quit"

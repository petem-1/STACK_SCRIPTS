#!/bin/bash

TS=$(date "+%m%d%Y%H%S")
TDS=$(date "+%m%d%Y")

#Check if 4 arguments are provided when running script
if [ $# -ne 4 ]; then
	echo "ERROR: Incorrect number of arguments."
	echo "Usage: $0 <command> <source_file> <backup_dir> <runner_name>"
	exit 1
fi

#Variables shifted to $2, $3, and $4 because $1 is the command
COMMAND=$1
SRC=$2
DST=$3
RUNNER=$4

#Logic Container: Defining the function
BACKUP_F_D() {

	#Builds the unique path
	BACKUP_PATH="${DST}/${RUNNER}/${TS}"

	echo "creating backup directory ${BACKUP_PATH}..."
	echo "Step 1: Creating unique timestamped directory at: ${BACKUP_PATH}"
	mkdir -p ${BACKUP_PATH}

	#Check if directory creation worked
	if [ $? -ne 0 ]; then
		echo "CRITICAL ERROR: Could not create directory ${BACKUP_PATH}. Stopping script."
		exit 1
	fi

	#Identifies if source is a file or directory
	if [[ -d ${SRC} ]]; then
		echo "${SRC} is a directory"
		echo "copying ${SRC} to backup location ${BACKUP_PATH}"
		cp -rf "${SRC}" "${BACKUP_PATH}"
	else
		echo "${SRC} is a file"
		echo "copying ${SRC} to backup location ${BACKUP_PATH}"
		cp -f "${SRC}" "${BACKUP_PATH}"
	fi

	if [ $? -ne 0 ]; then
		echo "ERROR: Copy failed for ${SRC}"
		exit 1
	fi
}
#MainBody
if [[ "$1" == "backup" ]]; then
	BACKUP_F_D
else
	echo "ERROR: Invalid command '$1'. Use 'backup'."
	exit1
fi

#validate
path=$(pwd)
echo "listing files in backup directory"
ls -ltr ${BACKUP_PATH}

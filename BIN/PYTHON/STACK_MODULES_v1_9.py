...
#database_import() - unzips a gzipped dumpfile and performs a Data Pump import into a target database
def database_import(runner, db_name, schema, directory, dumpfile):
	#Confirm which schema, dumpfile, and target database are being used for this import
	print("Importing schema {} from dump file {} into database {} for runner {}.".format(schema, dumpfile, db_name, runner))
	#Call G_Zip to unzip the gzipped dumpfile before impdp can read it
	G_Zip(dumpfile)
	...
	#Build the PAR file contents — each line sets a parameter that impdp reads at runtime
	par_contents="""userid=stack_temp/stackinc
	schemas={}
	remap_schema={}:{}_{}
	directory={}
	dumpfile={}
